/**
 * EXEC-021 smoke — Evidence Record State post-persist (Model C).
 * Writes SMOKE_021_PASS on success.
 */
...
import { INITIAL_REVISION_ID, createMemoryPersistenceSession } from "../packages/persistence/dist/index.js";
import {
...
const HUMAN = "human:smoke021";

try {
  if (referenceAppMarker.sprint < 21) throw new Error("marker sprint");
...
try {
  const ops = createResearchOperations(createMemoryPersistenceSession("smoke:021"));
  const evidenceId = "evidence:smoke021";
  await ops.registerEvidenceUnit({
    evidence_id: evidenceId,
    summary: "smoke 021",
    source: {
      source_class: "laboratory",
      source_locator: "lab://smoke021",
    ...
    created_at: AT,
    items: [
      { item_id: "eitem:smoke021", content_summary: "obs", item_state: "active" },
    ],
  });
  const r = await ops.transitionEvidenceRecordState({
    identity: evidenceId,
    revision_id: "rev:smoke-registered",
    expected_head_revision_id: INITIAL_REVISION_ID,
    append_event: true,
    transition: {
      to: "registered",
      authority_agent: HUMAN,
      reason: "smoke 021 register",
      decision_ref: "decision:smoke021",
      at: AT,
      event_id: "erte:smoke021",
    },
  });
  if (r.evidence.record_state !== "registered") throw new Error("record_state");
  if (r.head_revision_id !== "rev:smoke-registered") throw new Error("head");
  ...
  if (!events.some((e) => e.event_type === "ops.evidence_record_state_revision")) {
    throw new Error("event");
  }
  pass("evidence-record-state");
  pass("determinism");
  pass("event");
} catch (e) {
  fail("evidence-record-state flow", e);
}
...
if (process.exitCode) {
  console.error("SMOKE_021_FAIL");
  process.exit(1);
}
writeFileSync("SMOKE_021_PASS", results.join("\n") + "\n", "utf8");
console.log("SMOKE_021_PASS");
